import random
random.seed(2)#need to get deleted
...
def check_answer(answer):
    ...
    temp=[]
    for k in answer:
        if k[1:-1]==people.peoplelist[i].role:
            temp.append(True)
        if all(temp):
...
Here is the information you got:'''
    ...
    For example ({knight},{knight},{knight},{knight},{knight},{knight},{knight})\n''')
    input('Your answer\n____________\n')
if __name__ == "__main__":
    puzzle()
    a=check_solution(people.statementdict)
    print(len(a))
    print(a)